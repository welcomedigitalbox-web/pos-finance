...
export default function CashbookPage() {
  ...
  useEffect(() => {
    if (profile && !hasPermission(profile, "fin-cashbook")) router.replace("/");
  ...
  useEffect(() => {
    if (cashAccounts.length === 0 || accountId) return;
    const mine = cashAccounts.find((a) => a.store_id === storeId);
    setAccountId((mine || cashAccounts[0]).id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cashAccounts, storeId]);
  ...
  if (!profile || !hasPermission(profile, "fin-cashbook")) return null;
  ...
  return (
    <div className="pt-4">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="font-semibold text-lg">{t("fin_cashbookTitle")}</h2>
          {account && (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        >
          {cashAccounts.length === 0 && <option value="">{t("fin_selectAccount")}</option>}
          {cashAccounts.map((a) => (
